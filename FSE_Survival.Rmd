...
## Defining coxph model
run_coxph_model <- function(df.sample){
  coxph_model <- coxph(Surv(start_interval,
                            end_interval,
                            adopted == 1) ~
                         # act_committers # replaced by act_undir_committers
                       + log(n_commits+1)
                       + log(n_watchers+1)
                       + (other_badge > 0)
...
## defining save_model
save_model <- function(coxph_model, badge){
  sink(paste("FITS_FSE/","FIT_SA_",toString(badge),".txt", sep=""))
  print(summary(coxph_model))
  print(anova(coxph_model))
  print(vif(coxph_model))
  sink()
  save(coxph_model, file=paste('/data2/hemank/INPUT_SURVIVAL/FITS_FSE/FIT_',toString(badge),'_badge', sep=""))
  sink()
}
```
